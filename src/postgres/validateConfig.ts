...
import {postgresRunConfigSchema} from './postgresRunConfig.js';
...
export async function validatePostgresConfig(params: {
  configPath: string;
  logger: IForeLogger;
}): Promise<void> {
  ...
  try {
    ...
    const result = postgresRunConfigSchema.safeParse(data);
    if (result.success) {
      logger.log('✅ PostgreSQL configuration is valid');
      return;
    }
