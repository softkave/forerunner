...
/**
 * Keyfile generation config.
 *
 * Default `format: "mongodb"` produces a MongoDB-compatible keyFile (base64,
 * newline-terminated, permissions 0400).
 */
export interface KeyfileConfig {
  /** Output filepath for the keyfile. */
  path: string;
  /** Intended format/compat constraints (default: `"mongodb"`). */
  format?: 'mongodb' | 'generic';
  /** Number of random bytes to generate before encoding. */
  bytes?: number;
  /** Encoding for the generated bytes (default: `"base64"`). */
  encoding?: 'base64' | 'hex';
  /** File mode to apply (default: `0o400`). */
  mode?: number;
  /** Append trailing newline (default: `true`). */
  newline?: boolean;
}

/** Schema for `KeyfileConfig`. */
export const KeyfileConfigSchema: z.ZodType<KeyfileConfig> = z
  .object({
    path: z.string().min(1, 'Path is required'),
    format: z.enum(['mongodb', 'generic']).optional().default('mongodb'),
    bytes: z.number().int().positive().optional(),
    encoding: z.enum(['base64', 'hex']).optional().default('base64'),
    mode: z.number().int().positive().optional(),
    newline: z.boolean().optional().default(true),
  })
  .superRefine((val, ctx) => {
    if (val.format === 'mongodb' && val.encoding !== 'base64') {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'mongodb keyfile format requires base64 encoding',
        path: ['encoding'],
      });
    }
  });

/** CLI options for `certs keyfile`. */
export const GenerateKeyfileCLIOptionsSchema = z.object({
  force: z.boolean().optional(),
  config: z.string().min(1).optional(),
  cwd: z.string().optional(),
  silent: z.boolean().optional(),
  ...
  format: z.enum(['mongodb', 'generic']).optional(),
  bytes: z.number().int().positive().optional(),
...
/** CLI options accepted by `certs keyfile`. */
export interface GenerateKeyfileCLIOptions {
  /** Force regeneration even if keyfile already exists. */
  force?: boolean;
  /** Optional config filepath (when provided, flags are optional). */
  config?: string;
  /** Working directory (used to resolve relative paths). */
  ...
  /** Output path (flags mode). */
  path?: string;
  /** Keyfile format (default: mongodb). */
  format?: 'mongodb' | 'generic';
  /** Random bytes before encoding. */
  bytes?: number;
  /** Encoding for generated bytes. */
  encoding?: 'base64' | 'hex';
  /** File mode. */
  mode?: number;
